...
def intro():
    ...
    video_intro='Asset_Project_1/Space Enforcer Intro.mp4'
    # video_intro = Video(os.path.join("Asset_Project_1/Space Enforcer Intro.mp4"))
    
    Video(video_intro).resize(pygame.display.get_window_size())
    pygame.display.set_caption("Space Enforcer")
    #video_intro.play()
    Video(video_intro).toggle_mute() 
    pygame.mixer.Channel(1).play(pygame.mixer.Sound(os.path.join('Asset_Project_1', 'rocket-launch-collage.mp3')),fade_ms=10000)
    pygame.mixer.Channel(0).play(pygame.mixer.Sound(os.path.join('Asset_Project_1', 'Space_sound.mp3')),loops=-1,fade_ms=15000)
    
    while Video(video_intro).active:
      
        Video(video_intro).resize(pygame.display.get_window_size())
        key = None
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                Video(video_intro).stop()
                pygame.quit()
        ...
        if Video(video_intro).draw(DISPLAYSURF, (0, 0), force_draw=False):
            pygame.display.update()
